...
export function GithubCardComponent(properties, children) {
	...
	if (properties.user) {
		...
		const nRepos = h(`div#${cardUuid}-repos`, { class: "gc-forks" }, "0");
      ...
      fetch('https://api.github.com/users/${user}', { referrerPolicy: "no-referrer" }).then(response => response.json()).then(data => {
        ...
        document.getElementById('${cardUuid}-repos').innerText = Intl.NumberFormat('en-us', { notation: "compact", maximumFractionDigits: 1 }).format(data.public_repos).replaceAll("\u202f", '') + ' Repos';
        const avatarEl = document.getElementById('${cardUuid}-avatar');
        avatarEl.style.backgroundImage = 'url(' + data.avatar_url + ')';
        avatarEl.style.backgroundColor = 'transparent';
        document.getElementById('${cardUuid}-card').classList.remove("fetch-waiting");
        console.log("[GITHUB-CARD] Loaded card for user ${user} | ${cardUuid}.")
      }).catch(err => {
        const c = document.getElementById('${cardUuid}-card');
        c?.classList.add("fetch-error");
        console.warn("[GITHUB-CARD] (Error) Loading card for user ${user} | ${cardUuid}.")
      })
    `,
		);
		...
		return h(
			...
			[
				nTitle,
				nDescription,
				h("div", { class: "gc-infobar" }, [nFollowers, nRepos]),
				nScript,
			],
		);
	}
	...
	return h(
		"div",
		{ class: "hidden" },
		'Invalid directive. (Missing "repo" or "user" attribute)',
	);
}
